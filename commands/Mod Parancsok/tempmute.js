...
module.exports = {
    config: {
        name: "tempmute",
        aliases: "",
        usage: "<@felhasználó> idő indoklás",
    ...
    run: async (client, message, args) => {
        ...
        const timearg = args.slice(1, 2).join(' ');
        ...
        if (Number.isInteger(time)) {
            ...
            if (message.guild.member(user).roles.cache.has(tagrole)) {
                console.log("fasza2");
                message.guild.member(user).roles.cache.remove(tagrole);
                message.guild.member(user).roles.cache.add(muterole);
                setTimeout(function () {
                    message.guild.member(user).roles.cache.remove(muterole);
                    message.guild.member(user).roles.cache.add(tagrole);
                }, timetomute);
            ...
            **INDOKLÁS**: ${Reason}`);
            ...
            const notime = new Discord.MessageEmbed()
                ...
                .setAuthor(`${client.user.username} mute`, client.user.avatarURL({ format: 'png', dynamic: true, size: 2048 }))
                .setThumbnail(client.user.avatarURL({ format: 'png', dynamic: true, size: 2048 }))
                .setDescription(`Nem adtál meg időtartamot!`);
